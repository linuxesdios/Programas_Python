...
def check_key_presses():
    ...
    start_time = time.time()  # Tiempo en el que se inició la presión de la tecla
    pressed_duration = 0.0  # Duración de la tecla 'k' presionada
    while True:
        if keyboard.is_pressed('k'):
            # Incrementa la duración de la tecla 'q' presionada
            pressed_duration = time.time() - start_time

            if pressed_duration >= 1.0:  # Si se ha presionado 'k' durante al menos 1 segundo
                should_stop_recording = True
                break
        else:
            # Reinicia el contador si la tecla no está siendo presionada
